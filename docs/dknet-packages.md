...
**10.1.29** in `src/Directory.Packages.props`, and
`tests/DKNet.Templates.ScaffoldTests/PackageArchitectureTests.cs` guards those pins with four tests.
`AllDKNetPackageReferences_ShouldResolveToOneRelease` asserts that every `DKNet.*` pin resolves to the
*same* release, whichever release that is, so a partial bump fails the build while the pins stay in
lockstep at any single release. `AllDKNetPackages_ShouldBePinnedTo_10_1_29` then asserts that release
is exactly `10.1.29`, so a future bump moves all fourteen pins together *and* edits that one test. Two
further tests in the same file fail the build if a `DKNet.*` pin exists that no project references
(`EveryDKNetPackageVersion_ShouldBeReferencedByAtLeastOneProject`), or if any `PackageReference` under
`src/` carries an inline `Version` attribute (`NoPackageReference_ShouldCarryAVersionAttribute`).
...
| **DKNet.EfCore.Abstractions** | `AuditedEntity<TKey>`/`IAuditedProperties` base audit tracking, `[RaisesEvent]`/`[CrudCreate]`/`[CrudUpdate]`/`[SensitiveData]` declarative attributes | `Minimal.Domains/Share/DomainEntity.cs` extends `AuditedEntity<Guid>`; `[RaisesEvent]`, `[CrudCreate]`, `[CrudUpdate]`, `[SensitiveData]` on `Minimal.Domains/Features/AutomatedSample/Entities/Product.cs` | [docs/EfCore/DKNet.EfCore.Abstractions.md](https://github.com/baoduy/DKNet/blob/dev/docs/EfCore/DKNet.EfCore.Abstractions.md) |
| **DKNet.EfCore.AuditLogs** | `ICurrentUserProvider` — the name the audit-log capture asks for when stamping who changed an entity, plus structured audit-log entries beyond the four `CreatedBy`/`CreatedOn`/`UpdatedBy`/`UpdatedOn` fields | `Minimal.AppServices/Share/IPrincipalProvider.cs` extends `ICurrentUserProvider`; implemented by `GetCurrentUser()` in `Minimal.Api/Configs/Handlers/PrincipalProvider.cs` | [docs/EfCore/DKNet.EfCore.AuditLogs.md](https://github.com/baoduy/DKNet/blob/dev/docs/EfCore/DKNet.EfCore.AuditLogs.md) |
| **DKNet.EfCore.DataAuthorization** | `IDataOwnerProvider`/`IDataOwnerDbContext`, the `DataOwnerHook` that stamps the `OwnedBy` ownership key on save | `Minimal.AppServices/Share/IPrincipalProvider.cs` implements `IDataOwnerProvider`; `.AddDataOwnerProvider<CoreDbContext, PrincipalProvider>()` in `Minimal.Api/Configs/ServiceConfigs.cs`; `Minimal.Infra/Contexts/CoreDbContext.cs` implements `IDataOwnerDbContext`. See [auditing-and-data-ownership.md](./auditing-and-data-ownership.md) for the full save-pipeline story. | [docs/EfCore/DKNet.EfCore.DataAuthorization.md](https://github.com/baoduy/DKNet/blob/dev/docs/EfCore/DKNet.EfCore.DataAuthorization.md) |
