...
listed once, in [`docs/crud-attributes.md`](crud-attributes.md#what-the-generated-dto-holds) — including the
...
- The BDD host runs with `RequireAuthorization = false`, so `[FromClaim]` properties carry the
  built-in demonstration authentication provider's identity (`ClaimTypes.Name` =
  `SharedConsts.DemoAccount`) rather than a real caller's claim. Both `false`/`true` come from
  `Minimal.Api/appsettings.Testing.json` — the base `appsettings.json` ships
  `RequireAuthorization: true` and no `EnableDemoAuthentication` entry at all, and
  `TestApiFactoryBase` boots the host with `UseEnvironment("Testing")`.
  `BddApiFactory.AddFeatureOverrides` also sets the keys, but that in-memory entry is merged after
  `Program.cs` has already bound `FeatureOptions`, so the overlay file is what actually takes
  effect.
