...
- **Role-gated response properties** — `Product` declares
  ...
  **`DRK-1198-Demo-Product-With-Supplier-Data`**, with `supplierCostPrice` `42.50` and
  `supplierReferenceCode` `DRK-1198-DEMO-SUPPLIER-REF`; every other generated product leaves both
  properties null, so an empty payload on a random row proves nothing either way. Fetch it with
  `GET /v1/products?filter=Name:Equal:DRK-1198-Demo-Product-With-Supplier-Data`, and the item in the page
  reads:

  | Caller | `supplierCostPrice` | `supplierReferenceCode` |
  |---|---|---|
  | Unauthenticated — what a stock `dotnet run` gives you | absent | absent |
  | Authenticated, without the `pricing` role | absent | `"DRK-1198-DEMO-SUPPLIER-REF"` |
  | Authenticated, in the `pricing` role | `42.50` | `"DRK-1198-DEMO-SUPPLIER-REF"` |
