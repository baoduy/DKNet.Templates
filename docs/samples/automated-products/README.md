...
| `GET /` | Generic list (`DKNet.AspCore.Extensions`' `MapGetList`) with the uniform `filter`/`search`/`orderBy`/`desc`/`pageNumber`/`pageSize`/`fromDate`/`toDate` contract, resolved against `ProductDto`. `pageSize` defaults to 1000; `fromDate`/`toDate` bound last activity and, left out, default to the last three months — `Product` is audited, so a bare call is windowed. See [`docs/generic-list-endpoint.md`](../../generic-list-endpoint.md). |
