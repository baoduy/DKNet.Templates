...
| `GET /` | Generic list (`DKNet.AspCore.Extensions`' `MapGetList`) with the uniform `filter`/`search`/`orderBy`/`desc`/`pageNumber`/`pageSize` contract, resolved against `ProductDto`. On the pinned `10.1.19` `pageSize` defaults to `20` with a hard-coded ceiling of `100`; a release after that raises both to `1000` and adds `fromDate`/`toDate` last-activity bounds which, left out, window an audited listing — `Product` is audited — to the last three months. Figures and the version note: [`docs/generic-list-endpoint.md`](../../generic-list-endpoint.md#package-version). |
