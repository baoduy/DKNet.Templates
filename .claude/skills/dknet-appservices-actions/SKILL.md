...
sets `ByUser`; no endpoint stamps it by hand. With `RequireAuthorization` off, that authenticated
caller is the built-in demonstration authentication provider
(`FeatureManagement:EnableDemoAuthentication`), never an unauthenticated fallback. An authenticated caller whose token carries no
