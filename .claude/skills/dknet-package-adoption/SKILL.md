...
Each package below is independent — install only what the feature needs. All packages target **.NET 9.0+** (EF Core packages additionally need **EF Core 9.0+**); consult `src/Directory.Packages.props` (or your project's own central version file) before adding a version attribute per-project.
