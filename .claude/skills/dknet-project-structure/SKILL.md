...
- **`ByUser` / acting-user auto-fill**: `AddContextualRequestPopulation` (wired in `Program.cs`) populates any `[FromClaim(...)]`-decorated request property before validation and before the handler runs (see `CreatePurchaseOrderRequest.ByUser`) — no base class or inheritance is involved. A **generated** CRUD request can never carry a `[FromClaim]` property (the generator forwards only DataAnnotations attributes), so the automated sample's acting-user stamping goes through `DKNet.EfCore.DataAuthorization`'s `DataOwnerHook` instead, wired once in `ServiceConfigs.cs` and applying to every entity on `CoreDbContext`.
