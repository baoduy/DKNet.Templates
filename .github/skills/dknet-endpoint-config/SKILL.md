...
- **Generator-driven** — mirror `ProductV1Endpoint` (`AutomatedSample`): one `group.Map<Entity>Crud(o => …)` call, generated by `DKNet.SlimBus.Generators` from the entity's `[CrudCreate]`/`[CrudUpdate]` declarations, carrying per-route options and any by-name exclusion, with the routes the generator cannot express hand-mapped below it.
...
**Generator-driven** (`ProductV1Endpoint`) — one call, produced by `DKNet.SlimBus.Generators` from `Product`'s `[CrudCreate]`/`[CrudUpdate]` declarations, then whatever it cannot express:
...
`CrudMapOptions` is what makes the two styles coexist in one endpoint. `Exclude` drops a whole `CrudOp` or one route by its member name; `Configure` attaches any `RouteHandlerBuilder` setting to a `CrudOp` or to one named route. Full rules: `docs/crud-attributes.md`.
...
That is the whole endpoint for an entity whose every route the generator can express. The shipped
`Minimal.Api/ApiEndpoints/AutomatedSample/ProductV1Endpoint.cs` is one step past it: the same call
carrying per-route options and one `Exclude("Discontinue")`, followed by the two routes the generator
cannot express. The generated `Map{Entity}Crud()` extension (in the `Minimal.AppServices.Crud`
namespace, produced by the `[CrudCreate]`/`[CrudUpdate]` on the entity) registers every route it is
not told to skip.
...
| | `PurchaseOrderV1Endpoint` (hand-mapped) | `ProductV1Endpoint` (generated first, hand-written below) |
|---|---|---|
| Base route | `/v1/purchase-orders` | `/v1/products` |
| Routes | `POST /`, `GET /`, `GET {id}`, `PUT {id}`, `POST {id}/cancel`, `DELETE {id}` — every one a literal `group.Map*` call | `group.MapProductCrud(o => …)` — GetById/GetList/Create/ChangePrice/Delete/Approve/AssignSupplierReference generated, plus `PUT {id}/discontinue` and `GET /summary` hand-mapped below it |
| Idempotency | `.RequiredIdempotentKey()` on create; clients send `X-Idempotency-Key: {Guid}` | None on the generated create route — add one by excluding `"Create"` and hand-mapping it |
