...
2. **Validation** — FluentValidation validators alongside their request record; fail fast, use `.When(...)` for conditional rules. A validator runs on generated routes as well as hand-mapped ones — `UseEndpointConfigs` applies `AddFluentValidationAutoValidation()` to every endpoint group (`Program.cs:50`), which is how the product sample refuses a duplicate name and a delete of a product still for sale, both `409`. What is *not* evaluated on a generated route is a forwarded DataAnnotations attribute; see the comparison doc's "Request validation" row before relying on `[Range]`/`[Required]` on a `[CrudCreate]`/`[CrudUpdate]` parameter.
