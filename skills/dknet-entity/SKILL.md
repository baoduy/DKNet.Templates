...
1. the `dknet-domain-entity` skill
...
3. The exemplar for the selected mode:
   ...
   - `auto` — `ApiEndpoints/Minimal.Domains/Features/AutomatedSample/Entities/Product.cs` (class-level `[RaisesEvent]`, `[CrudCreate]` ctor, `[CrudUpdate]` and `[CrudAction]` methods; no `AddEvent` anywhere) plus the `dknet-domain-entity` skill
...
1. Use the `dknet-implementer` subagent (via the Agent tool) to execute Steps 1–4 of the implementer protocol: domain entity, schema constant, owned types, EF Core mapper, optional sequence/seed data, then `dotnet ef migrations add <Name> -c CoreDbContext -p Minimal.Infra/Minimal.Infra.csproj` from `ApiEndpoints/`.
